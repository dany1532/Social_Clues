...
public class UISpriteAnimation : MonoBehaviour
{
	...
	void Update ()
	{
		if (mActive && mSpriteNames.Count > 0 && Application.isPlaying && mFPS > 0f)
		{
			...
			if (rate < mDelta)
			{
				...
				if(!mPing_Pong){
					...
					if(mActive){
						mPongIndex = (int) Mathf.PingPong(mIndex++,mSpriteNames.Count-1);
						mPongIndex = Mathf.Clamp(mPongIndex, 0, mSpriteNames.Count-1);
						ChangeSprite(mPongIndex);
					}
				}
	...
	public int SpriteListSize(){
		return mSpriteNames.Count;
	}
	
	public void UIUSetPingPong(bool isPong){
		mPing_Pong = isPong;	
	}
	...
	virtual protected void RebuildSpriteList ()
	{
	...
	virtual protected void ChangeSprite (int index)
	{
		mSprite.spriteName = Utilities.SetTextureName(mSpriteNames[index]);
	}
}
