...
public class UIScrollBar : MonoBehaviour
{
	...
	void OnDragForeground (GameObject go, Vector2 delta)
	{
		...
		//Vector3 newLocation = cameraToUpdate.transform.localPosition + new Vector3(0, delta.y, 0);
		//cameraToUpdate.transform.localPosition = newLocation;
	...
	public void updateCamera() {
		...
		// scrollvalue = 1f - (min / sum)
		// scrollvalue - 1f = - (min / sum)
		// (scrollvalue - 1f) * -1 = min / sum
		// ((scrollvalue - 1f) * -1) * sum = min
		
		float min = (((this.scrollValue - 1f) * -1) * sum);
		//Debug.Log ("min: " + min);
		
		//Bounds panelBounds = NGUIMath.CalculateAbsoluteWidgetBounds(npcContainer.gameObject.transform);
		Bounds panelBounds = NGUIMath.CalculateAbsoluteWidgetBounds(npcContainer.transform);
		float height = panelBounds.max.y - panelBounds.min.y;
		
		min *= height;
		//Debug.Log ("min2: " + min);
		
		float extents = Math.Abs(cameraToUpdate.rect.yMax - cameraToUpdate.rect.yMin);
		// min = cameraMin.y - extents - panelBounds.min.y
		// cameraMin.y = min + extents + panelBounds.y		
		float yValue = min + extents + panelBounds.min.y;
		//Debug.Log ("yValue: " + yValue);
		
		Vector3 newLocation = new Vector3(cameraToUpdate.transform.position.x, yValue, cameraToUpdate.transform.position.z);
		cameraToUpdate.transform.position = newLocation;		
		
		//Debug.Log ("camera position: " + cameraToUpdate.transform.position);
		
		//UIScrollBar.scrollValue;
		/*
