...
static public class NGUIMath
{
	...
	static public Bounds CalculateAbsoluteWidgetBounds (Transform trans, int layer)
	{
		UIWidget[] widgets = trans.GetComponentsInChildren<UIWidget>() as UIWidget[];
		if (widgets.Length == 0) return new Bounds(trans.position, Vector3.zero);
		...
		{			
			...
			if (w.gameObject.layer != layer) continue;
	...
	/// <summary>
	/// Color.Slerp(from, to, Time.deltaTime * strength) is not framerate-independent. This function is.
	/// </summary>

	static public Color SpringLerp (Color from, Color to, float strength, float deltaTime)
	{
		return Color.Lerp(from, to, SpringLerp(strength, deltaTime));
	}
