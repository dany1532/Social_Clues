...
public class UIButtonTween : MonoBehaviour
{
	...
	public bool repositionFix = false;
	...
	void Start () {
		...
		if(hasScroll) {
			cameraObject = GameObject.Find ("NPCDataCamera");
			delayedClick ();
			Invoke ("delayedClick", 0.1f);
		}
	}
	void delayedClick ()
	{
		...
		if(repositionFix) {
			//Invoke("this.transform.parent.transform.parent.gameObject.GetComponent<UITable>().Reposition()", 0.1f);
			this.transform.parent.transform.parent.gameObject.GetComponent<UITable>().Invoke ("Reposition", 1.0f);
		}
		
		if(hasScroll) {
			GameObject.Find ("NPCDataCamera").GetComponent<UIDraggableCamera>().Invoke("updateScrollBar", 1.0f);
		}		
	}	
	void delayedPlay() {
		if (enabled && trigger == Trigger.OnClick)
	...
	void OnClick ()
	{
		...
		*/
		if(hasScroll) {
			// requires delay because tween takes time to resize
			GameObject.Find ("NPCDataCamera").GetComponent<UIDraggableCamera>().Invoke("updateScrollBar", 0.4f);
		}		
	}
