...
public class UIStretch : MonoBehaviour
{
	public enum Style
	{
		...
		BasedOnHeightKeepingRatioPerc,
		BasedOnHeightKeepingRatioExact,
		BasedOnWidthKeepingRatioExact,
		BasedOnHeightKeepingRatioPercAcross = 64
	}
	...
	public Style style = Style.BasedOnHeightKeepingRatioPerc;
	...
	public  void UpdateStretch()
	{
		...
		if (style != Style.None)
		{
			...
			else if (style == Style.BasedOnHeightKeepingRatioPerc)
			{
				float imageRatio = initialSize.x / initialSize.y;
				
				localScale.x = relativeSize.y * rectHeight * imageRatio;
				localScale.y = relativeSize.y * rectHeight;
			}
			else if (style == Style.BasedOnHeightKeepingRatioPercAcross)
			{
				float imageRatio = initialSize.x / initialSize.y;
				
				localScale.x = relativeSize.y * rectHeight * imageRatio * 768 / uiCamera.pixelHeight;
				localScale.y = relativeSize.y * rectHeight * 768 / uiCamera.pixelHeight;
			}
			else if (style == Style.BasedOnHeightKeepingRatioExact)
			{
				if (runOnlyOnce)
				{
					...
					if (texture != null)
					{
						...
						{
							initialSize.x = texture.mainTexture.width;
							initialSize.y = texture.mainTexture.height;
						}
					}
					else
					{
						...
						if (sprite != null)
						{
							initialSize.x = sprite.mInner.width;
							initialSize.y = sprite.mInner.height;
						}
					}
				}
				
				float imageRatio = initialSize.x / initialSize.y;
				float spriteRatio = relativeSize.y / 768.0f;
				
				localScale.y = spriteRatio * rectHeight;
				localScale.x = localScale.y * imageRatio;
			}
			else if (style == Style.BasedOnWidthKeepingRatioExact)
			{
				float imageRatio = initialSize.y / initialSize.x;
				
				localScale.x = relativeSize.x * rectWidth;
				localScale.y = relativeSize.x * rectWidth * imageRatio;
			}
