...
public class UILabel : UIWidget
{
	...
	public bool checkNumberOfLines = false;
	...
	/*
	public bool checkNumberOfLines {
		get {
			return checkNumberOfLines;
		} set {
			checkNumberOfLines = value;
		}
	}
	*/
	/// <summary>
		...
		if (scale > 0f)
		{
			for (;;)
			{
				...
				else if (mMaxLineWidth > 0)
				{
					...
					//Debug.Log ("1: " + mProcessedText);
					if(checkNumberOfLines) {
						numberOfLines = mFont.getNumberOfLines(mText, mMaxLineWidth / scale, mShrinkToFit ? 0 : mMaxLineCount, mEncoding, mSymbols);
						//Debug.Log ("numberOfLines: " + numberOfLines);
					}					
				}
				else if (!mShrinkToFit && mMaxLineCount > 0)
				{
					mProcessedText = mFont.WrapText(mText, 100000f, mMaxLineCount, mEncoding, mSymbols);
					//Debug.Log ("2: " + mProcessedText);
				}
				else
				{
